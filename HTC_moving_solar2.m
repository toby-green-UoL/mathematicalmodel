...
if light_source_user_defined == true
    direct_normal_irradiance = 1000; % [W/m^2]
    diffuse_irradiance = 300; % [W/m^2]
    zenith_degrees = 10; % the angle between the sun and the vertical
...
rays_aperture_min_x = -4;
rays_aperture_max_x = 4;
rays_aperture_min_y = -0.5;
rays_aperture_max_y = 0.5;
rays_aperture_z = 0;
...
cp_reactor = 385; % J per kg per K
cp_water = 4181; % liquid, J per kg per K
mix_water = 0.8; %for batch
density_reactor = 8960; % kg per m^3
density_water = 997; % kg per m^3
conductivity_reactor = 38.4; % W / (m K) reduced by a factor of 10
cp_biomass = 2380; %oak
density_biomass = 740; % Oak kg per m^3 
mix_biomass = 1 - mix_water; %for batch

h_c_air_reactor = 13.5; % convection to air, W /(m^2 K)
                       %https://www.engineeringtoolbox.com/overall-heat-transfer-coefficients-d_284.html
h_c_water_reactor = 400; % convection to water, W /(m^2 K)% Comparison of Heat Transfer Coefficients in Free and ForcedConvection using Circular Annular Finned Tubes, 
                            %Dr. Abdul Jabbar, IJAIEM, 5,4, 2016 
